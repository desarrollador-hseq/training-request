...
const MainLayout = async ({ children }: { children: ReactNode }) => {
  ...
  return (
    <CollaboratorsCartProvider>
      <main className={cn("relative flex flex-col h-full min-h-screen m-0 p-0 mx-auto bg-blue-100/50", roboto.className)}>
        <DashboardNavbar
        ...
        <div className="mt-1 md:pl-[223px] min-h-screen xl:flex justify-center items-start xl:w-full relative">
          <div className="mx-1 min-h-full mt-[56px] max-w-[1200px] w-full">
            {session.user.isValid ? children : <NotValidCompany email={session.user.email} />}
          </div>
        </div>
        <ScrollUp />
      </main>
